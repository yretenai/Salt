...
public abstract record BaseChunk(RIFFAtom Atom, FEVSoundBank Bank) {
	...
	protected static BaseChunk ReadChunk(FEVReader reader, RIFFAtom atom, FEVSoundBank soundBank) {
		switch (atom.Id) {
			...
			case ChunkId.PROP: {
				return new PropertyChunk(reader, atom, soundBank);
			}
			...
			case ChunkId.WAIB: {
				return new WaveformInstrumentBodyChunk(reader, atom, soundBank);
			}
			case ChunkId.WAIS: throw new UnreachableException();
			case ChunkId.WAIT: {
				return new WaveformInstrumentChunk(reader, atom, soundBank);
			}
			case ChunkId.WAV: {
